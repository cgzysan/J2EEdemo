	...
	<div id="MainArea">
		<table class="MainArea_Content" align="center" cellspacing="0"
			...
			<tbody id="TableData">
				...
				<c:forEach var="food" items="${requestScope.list }" varStatus="vs">
					<tr class="TableDetail1">
						<td>${vs.count }&nbsp;</td>
						<td>${food.foodName }&nbsp;</td>
						<c:forEach var="type" items="${requestScope.types }">
							<c:if test="${food.foodType_id==type.id }">
								<c:set var="ty" value="${type.typeName }" />
							</c:if>
						</c:forEach>
						<td><c:out value="${ty }"></c:out>&nbsp;</td>
						<td>${food.price }&nbsp;</td>
						<td>${food.mprice }&nbsp;</td>
						<td><a
							href="${pageContext.request.contextPath }/food?method=show&id=${food.id }"
							class="FunctionButton">更新</a> <a
							href="${pageContext.request.contextPath }/food?method=delete&id=${food.id}"
		...
		<div id="TableTail" align="center">
			<div class="FunctionButton">
				<a
					href="${pageContext.request.contextPath }/food?method=findFoodType">添加</a>
			</div>
			<div>
