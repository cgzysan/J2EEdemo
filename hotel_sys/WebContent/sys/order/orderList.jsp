...
<body>
	...
	<div id="MainArea">
		<table class="MainArea_Content" align="center" cellspacing="0" cellpadding="0">
			...
			<!--显示数据列表 -->
			<tbody id="TableData">
				<c:forEach var="o" items="${requestScope.pageBean.pageData }">
					<tr height="60">
				 		<td>${o.id }</td>
				 		<c:forEach var="table" items="${applicationScope.table }">
				 			<c:if test="${o.table_id==table.id }">
				 				<c:set var="tableName" value="${table.typeName }"/>
				 			</c:if>
				 		</c:forEach>
				 		...
				 		<td>
							<a href="${pageContext.request.contextPath }/order?method=getOrderDetail&orderId=${o.id}" class="FunctionButton">详细</a> 
			 				<a href="${pageContext.request.contextPath }/order?method=pay&tableId=${o.table_id}&orderId=${o.id}" class="FunctionButton">结账</a>
				 		</td>
			 		</tr>
				</c:forEach>
			</tbody>
